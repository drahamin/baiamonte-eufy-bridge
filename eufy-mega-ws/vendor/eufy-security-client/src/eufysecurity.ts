...
export class EufySecurity extends TypedEmitter<EufySecurityEvents> {
  private readonly SNAPSHOT_CACHE_MAX_BYTES = 10 * 1024 * 1024;
  ...
  protected async _initializeInternals(): Promise<void> {
    ...
    if (this.config.snapshotCache === undefined) {
      this.config.snapshotCache = false;
    }
  ...
  private getSnapshotCacheFile(deviceSN: string): string {
    const safeSerial = deviceSN.replace(/[^A-Za-z0-9_.-]/g, "_");
    return path.join(this.config.persistentDir!, "snapshots", `${safeSerial}.img`);
  }

  private getCachedSnapshotType(data: Buffer): Picture["type"] | undefined {
  ...
  private restoreCachedSnapshot(device: Device): void {
    if (!this.config.snapshotCache || !device.hasProperty(PropertyName.DevicePicture)) return;

    const file = this.getSnapshotCacheFile(device.getSerial());
    try {
      const fileStat = statSync(file);
      if (!fileStat.isFile() || fileStat.size <= 0 || fileStat.size > this.SNAPSHOT_CACHE_MAX_BYTES) return;
      const data = readFileSync(file);
      const type = this.getCachedSnapshotType(data);
      if (!type) {
        rootMainLogger.warn("Snapshot cache ignored an unsupported image", { deviceSN: device.getSerial(), file });
        return;
      }
      device.updateProperty(PropertyName.DevicePicture, { data, type } as Picture, true);
      rootMainLogger.debug("Restored cached event snapshot", { deviceSN: device.getSerial(), file });
    } catch (err) {
      const error = ensureError(err);
      // A cache miss is normal on the first event; only surface it at debug level.
      rootMainLogger.debug("No cached event snapshot available", {
        error: getError(error),
        deviceSN: device.getSerial(),
      });
    }
  ...
  private cacheSnapshot(device: Device, value: PropertyValue): void {
    ...
    try {
      ...
      writeFileSync(temporaryFile, picture.data, { mode: 0o600 });
      renameSync(temporaryFile, file);
      rootMainLogger.debug("Cached push event snapshot", {
        deviceSN: device.getSerial(),
        bytes: picture.data.length,
      });
    } catch (err) {
      const error = ensureError(err);
      rootMainLogger.warn("Unable to cache push event snapshot", {
        error: getError(error),
        deviceSN: device.getSerial(),
      });
    }
  ...
  private onDevicePropertyChanged(device: Device, name: string, value: PropertyValue, ready: boolean): void {
    try {
      ...
      if (name === PropertyName.DevicePicture) {
        this.cacheSnapshot(device, value);
      }
      if (
