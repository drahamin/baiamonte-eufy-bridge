...
// Push notifications can arrive a few seconds before the thumbnail object is visible on Eufy's
// S3-backed endpoint. Retrying here keeps that race out of every device-specific push handler.
const EVENT_IMAGE_RETRY_DELAYS_MS = [0, 1000, 3000] as const;
const wait = (milliseconds: number): Promise<void> =>
  new Promise((resolve) => {
    setTimeout(resolve, milliseconds);
  });
const redactSignedImageUrl = (value: string): string => {
  try {
    ...
    if (this.connected) {
      ...
      if (device) {
        ...
        if (station) {
          ...
          for (const [attempt, delayMs] of EVENT_IMAGE_RETRY_DELAYS_MS.entries()) {
            ...
            try {
              ...
              if (response.status == 200) {
                const decoded = await decodeImageAsync(station.p2p_did, response.data as Buffer);
                if (decoded.length > 0) return decoded;
                lastError = new Error("event image response was empty");
              } else {
                lastError = new Error(`event image returned HTTP ${response.status} ${response.statusText}`);
              }
            } catch (err) {
              lastError = ensureError(err);
            }

            if (attempt < EVENT_IMAGE_RETRY_DELAYS_MS.length - 1) {
              rootHTTPLogger.debug("Get Image - thumbnail not ready, retrying", {
                deviceSN,
                attempt: attempt + 1,
                nextDelayMs: EVENT_IMAGE_RETRY_DELAYS_MS[attempt + 1],
              });
            }
          }
          ...
          if (lastError) {
            rootHTTPLogger.error("Get Image - retries exhausted", {
              ...
              attempts: EVENT_IMAGE_RETRY_DELAYS_MS.length,
            });
          }
        }
      }
