...
describe("MegaHTTPApi", () => {
  ...
  describe("signed requests (via registerPushToken)", () => {
    ...
    it("rekeys and retries once when a persisted identity is rejected", async () => {
      const { api, requests } = await makeApi([
        { statusCode: 200, body: JSON.stringify({ code: 4404, msg: "get identity error" }) },
        { statusCode: 200, body: JSON.stringify({ code: 0, msg: "success!" }) },
      ]);
      api.setAuth("authtok", "user456");

      const result = await api.registerPushToken("fcmTOKEN123");

      expect(result.code).toBe(0);
      expect(requests).toHaveLength(2);
      expect(MegaHTTPApi.prototype.keyExchange).toHaveBeenCalledTimes(2);
      expect(requests[0].headers["x-request-once"]).not.toBe(requests[1].headers["x-request-once"]);
    });

    it("does not loop when the replacement identity is also rejected", async () => {
      const { api, requests } = await makeApi([
        { statusCode: 200, body: JSON.stringify({ code: 4404, msg: "get identity error" }) },
        { statusCode: 200, body: JSON.stringify({ code: 4404, msg: "get identity error" }) },
      ]);
