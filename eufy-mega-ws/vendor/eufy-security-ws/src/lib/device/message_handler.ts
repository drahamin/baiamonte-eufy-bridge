...
export class DeviceMessageHandler {
  ...
  static addStreamingDevice(stationSN: string, client: Client): void {
    if (
      DeviceMessageHandler.streamingDevices[stationSN] !== undefined &&
      !DeviceMessageHandler.streamingDevices[stationSN].includes(client)
    )
  ...
  static addDownloadingDevice(stationSN: string, client: Client): void {
    if (
      DeviceMessageHandler.downloadingDevices[stationSN] !== undefined &&
      !DeviceMessageHandler.downloadingDevices[stationSN].includes(client)
    )
  ...
  static addTalkbackingDevice(stationSN: string, client: Client): void {
    if (
      DeviceMessageHandler.talkbackingDevices[stationSN] !== undefined &&
      !DeviceMessageHandler.talkbackingDevices[stationSN].includes(client)
    )
  ...
  static async handle(
    ...
    switch (command) {
      ...
      case DeviceCommand.getVoices:
        if (client.schemaVersion >= 3) {
          const voices = await driver
            .getApi()
            .getVoices(device.getSerial())
            .catch((error) => {
              throw error;
            });
      ...
      case DeviceCommand.getUsers:
        if (client.schemaVersion >= 13) {
          const users = await driver
            .getApi()
            .getUsers(device.getSerial(), device.getStationSerial())
            .catch((error) => {
              throw error;
            });
