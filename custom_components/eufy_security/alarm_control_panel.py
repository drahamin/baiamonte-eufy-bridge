...
async def async_setup_entry(
    ...
    entities = [
        EufySecurityAlarmControlPanel(coordinator, metadata)
    ...
    platform.async_register_entity_service(
        "alarm_arm_custom1", {}, "async_alarm_arm_custom_bypass"
    )
    platform.async_register_entity_service(
        "alarm_arm_custom2", {}, "async_alarm_arm_night"
    )
    platform.async_register_entity_service(
        "alarm_arm_custom3", {}, "async_alarm_arm_vacation"
    )
    platform.async_register_entity_service("geofence", {}, "geofence")
    platform.async_register_entity_service("schedule", {}, "schedule")
    platform.async_register_entity_service(
        "chime", Schema.CHIME_SERVICE_SCHEMA.value, "chime"
    )
...
class EufySecurityAlarmControlPanel(AlarmControlPanelEntity, EufySecurityEntity):
    ...
    def alarm_state(self):
        ...
        current_mode = get_child_value(
            self.product.properties, self.metadata.name, False
        )
        ...
        current_mode = get_child_value(
            ...
            self.metadata.name,
            CurrentModeToState(self.guard_mode),
        )
