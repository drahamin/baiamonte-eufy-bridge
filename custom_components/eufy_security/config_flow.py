...
class EufySecurityOptionFlowHandler(config_entries.OptionsFlow):
    ...
    def __init__(self, config_entry: ConfigEntry) -> None:
        ...
        self.schema = vol.Schema(
            {
                vol.Optional(
                    ConfigField.sync_interval.name, default=self.config.sync_interval
                ): vol.All(vol.Coerce(int), vol.Range(min=5, max=86400)),
                vol.Optional(
                    ConfigField.rtsp_server_address.name,
                    default=self.config.rtsp_server_address,
                ): str,
                vol.Optional(
                ...
                vol.Optional(
                    ConfigField.name_for_custom1.name,
                    default=self.config.name_for_custom1,
                ): str,
                vol.Optional(
                    ConfigField.name_for_custom2.name,
                    default=self.config.name_for_custom2,
                ): str,
                vol.Optional(
                    ConfigField.name_for_custom3.name,
                    default=self.config.name_for_custom3,
                ): str,
            }
        )
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            valid = await self._test_credentials(
                user_input[ConfigField.host.name], user_input[ConfigField.port.name]
            )
            if valid:
                return self.async_create_entry(
                    title=user_input[ConfigField.host.name], data=user_input
                )
    ...
    async def _show_config_form(self, user_input):  # pylint: disable=unused-argument
        return self.async_show_form(
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(
                        ConfigField.host.name, default=ConfigField.host.value
                    ): str,
                    vol.Required(
                        ConfigField.port.name, default=ConfigField.port.value
                    ): vol.All(vol.Coerce(int), vol.Range(min=1, max=65535)),
                }
            ),
            errors=self._errors,
    ...
    async def _test_credentials(self, host, port):  # pylint: disable=unused-argument
        try:
            ...
            api_client: ApiClient = ApiClient(
                config, aiohttp_client.async_get_clientsession(self.hass), None
            )
            ...
            _LOGGER.error(
                "Unable to connect to Baiamonte eufy Bridge: %s", type(ex).__name__
            )
        return False
            ...
            if coordinator.config.mfa_required is True:
                return self.async_show_form(
                    ...
                    description_placeholders={
                        "captcha_img": "Enter the multi-factor authentication code"
                    },
                )
            else:
                return self.async_show_form(
                    ...
                    description_placeholders={
                        "captcha_img": '<img id="eufy_security_captcha" src="'
                        + coordinator.config.captcha_img
                        + '"/>'
                    },
                )
