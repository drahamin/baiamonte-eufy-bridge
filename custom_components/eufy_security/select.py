...
async def async_setup_entry(
    ...
    product_properties = get_product_properties_by_filter(
        ...
        PlatformToPropertyType[Platform.SELECT.name].value,
    )
    entities = [
        EufySelectEntity(coordinator, metadata) for metadata in product_properties
    ]
...
class EufySelectEntity(SelectEntity, EufySecurityEntity):
    ...
    async def async_select_option(self, option: str):
        to_value = [
            key for key, value in self.metadata.states.items() if value == option
        ][0]
