...
from .const import EventSourceType, MessageField
...
class OutgoingMessageType(Enum):
    ...
    start_listening = {
        ...
        MessageField.DOMAIN: EventSourceType.server,
    }
    set_api_schema = {
        MessageField.DUMMY: auto(),
        MessageField.DOMAIN: EventSourceType.server,
        MessageField.SCHEMA_VERSION: None,
    }
    ...
    pan_and_tilt = {
        ...
        MessageField.DIRECTION: None,
    }
    preset_position = {
        MessageField.DUMMY: auto(),
        MessageField.DOMAIN: EventSourceType.device,
        MessageField.POSITION: None,
    }
...
class OutgoingMessage:
    ...
    def __init__(self, message_type: OutgoingMessageType, **kwargs) -> None:
        ...
        for key in message_type.value.keys():
            try:
                self._message[key.value] = kwargs.get(
                    OutgoingMessageToParameter[key.value].value
                )
            except KeyError:
                pass
        ...
        if default_domain in [
            ...
            EventSourceType.station,
            EventSourceType.device,
        ]:
            self._message[MessageField.SERIAL_NO.value] = kwargs.get(
                OutgoingMessageToParameter[MessageField.SERIAL_NO.value].value
            )

        domain = (
            default_domain.value
            if default_domain != EventSourceType.product
            else kwargs.get(MessageField.DOMAIN.value, "")
        )
        command = (
            self.type.name
