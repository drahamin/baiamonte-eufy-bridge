...
class ApiClient:
    ...
    def __init__(
        self, config, session: aiohttp.ClientSession, on_error_callback
    ) -> None:
        self._config = config
        ...
        self._captcha_future: asyncio.Future[dict] = (
            asyncio.get_running_loop().create_future()
        )
        self._mfa_future: asyncio.Future[dict] = (
            asyncio.get_running_loop().create_future()
        )
    ...
    async def _check_interactive_mode(self):
        ...
        try:
            ...
            _LOGGER.debug("_check_interactive_mode 4")
            await self._connect_driver()
            raise DriverNotConnectedException() from exc

    async def _set_products(self) -> None:
        _LOGGER.debug("_set_products 1")
        self._captcha_future = asyncio.get_running_loop().create_future()
        self._mfa_future = asyncio.get_running_loop().create_future()
        result = await self._start_listening()
        _LOGGER.debug("_set_products 2")

        if (
            result[MessageField.STATE.value][EventSourceType.driver.name][
                MessageField.CONNECTED.value
            ]
            is False
        ):
            await self._check_interactive_mode()

        self._devices = await self._get_products(
            ProductType.device, result[MessageField.STATE.value]["devices"]
        )
        self._stations = await self._get_products(
            ProductType.station, result[MessageField.STATE.value]["stations"]
        )
    ...
    async def _start_listening(self):
        return await self._send_message_get_response(
            OutgoingMessage(OutgoingMessageType.start_listening)
        )

    async def _set_schema(self, schema_version: int) -> None:
        await self._send_message_get_response(
            OutgoingMessage(
                OutgoingMessageType.set_api_schema, schema_version=schema_version
            )
        )
    ...
    async def _disconnect_driver(self) -> None:
        await self._send_message_get_response(
            OutgoingMessage(OutgoingMessageType.disconnect)
        )

    async def _connect_driver(self) -> None:
        await self._send_message_get_response(
            OutgoingMessage(OutgoingMessageType.connect)
        )

    async def set_log_level(self, log_level: str) -> None:
        """set log level of websocket server"""
        await self._send_message_get_response(
            OutgoingMessage(OutgoingMessageType.set_log_level, log_level=log_level)
        )
    ...
    async def _set_captcha(self, captcha_id: str, captcha_input: str) -> None:
        await self._send_message_get_response(
            OutgoingMessage(
                OutgoingMessageType.set_captcha,
    ...
    async def _set_mfa_code(self, mfa_input: str) -> None:
        await self._send_message_get_response(
            OutgoingMessage(OutgoingMessageType.set_verify_code, verify_code=mfa_input)
        )
    ...
    async def _get_metadata(self, product_type: ProductType, serial_no: str) -> dict:
        result = await self._send_message_get_response(
            OutgoingMessage(
                OutgoingMessageType.get_properties_metadata,
                domain=product_type.name,
                serial_no=serial_no,
            )
        )
    ...
    async def _get_properties(self, product_type: ProductType, serial_no: str) -> dict:
        result = await self._send_message_get_response(
            OutgoingMessage(
                OutgoingMessageType.get_properties,
                domain=product_type.name,
                serial_no=serial_no,
            )
        )
    ...
    async def set_property(
        self, product_type: ProductType, serial_no: str, name: str, value: Any
    ) -> None:
    ...
    async def trigger_alarm(
        self, product_type: ProductType, serial_no: str, duration: int
    ) -> None:
    ...
    async def reset_alarm(self, product_type: ProductType, serial_no: str) -> None:
        ...
        await self._send_message_get_response(
            OutgoingMessage(
                OutgoingMessageType.reset_alarm,
                domain=product_type.name,
                serial_no=serial_no,
            )
        )
    ...
    async def pan_and_tilt(
        self, product_type: ProductType, serial_no: str, direction: int
    ) -> None:
        """Process start pan tilt rotate zoom"""
        await self._send_message_get_response(
            OutgoingMessage(
                OutgoingMessageType.pan_and_tilt,
                serial_no=serial_no,
                direction=direction,
            )
        )

    async def preset_position(
        self, product_type: ProductType, serial_no: str, position: int
    ) -> None:
        """Process preset position call"""
        await self._send_message_get_response(
            OutgoingMessage(
                OutgoingMessageType.preset_position,
                serial_no=serial_no,
                position=position,
            )
        )

    async def save_preset_position(
        self, product_type: ProductType, serial_no: str, position: int
    ) -> None:
        """Process save preset position call"""
        await self._send_message_get_response(
            OutgoingMessage(
                OutgoingMessageType.save_preset_position,
                serial_no=serial_no,
                position=position,
            )
        )

    async def delete_preset_position(
        self, product_type: ProductType, serial_no: str, position: int
    ) -> None:
        """Process delete preset position call"""
        await self._send_message_get_response(
            OutgoingMessage(
                OutgoingMessageType.delete_preset_position,
                serial_no=serial_no,
                position=position,
            )
        )

    async def start_rtsp_livestream(
        self, product_type: ProductType, serial_no: str
    ) -> None:
        """Process start rtsp livestream call"""
        await self._send_message_get_response(
            OutgoingMessage(
                OutgoingMessageType.start_rtsp_livestream, serial_no=serial_no
            )
        )
    ...
    async def calibrate(self, product_type: ProductType, serial_no: str) -> None:
        ...
        await self._send_message_get_response(
            OutgoingMessage(OutgoingMessageType.calibrate, serial_no=serial_no)
        )

    async def stop_rtsp_livestream(
        self, product_type: ProductType, serial_no: str
    ) -> None:
        """Process stop rtsp livestream call"""
        await self._send_message_get_response(
            OutgoingMessage(
                OutgoingMessageType.stop_rtsp_livestream, serial_no=serial_no
            )
        )

    async def _get_is_rtsp_streaming(
        self, product_type: ProductType, serial_no: str
    ) -> bool:
        result = await self._send_message_get_response(
            OutgoingMessage(
                OutgoingMessageType.is_rtsp_livestreaming, serial_no=serial_no
            )
        )
        return result[MessageField.LIVE_STREAMING.value]

    async def _get_is_connected(
        self, product_type: ProductType, serial_no: str
    ) -> bool:
        result = await self._send_message_get_response(
            OutgoingMessage(OutgoingMessageType.is_connected, serial_no=serial_no)
        )
    ...
    async def start_livestream(self, product_type: ProductType, serial_no: str) -> None:
        ...
        await self._send_message_get_response(
            OutgoingMessage(OutgoingMessageType.start_livestream, serial_no=serial_no)
        )

    async def stop_livestream(self, product_type: ProductType, serial_no: str) -> None:
        """Process stop p2p livestream call"""
        await self._send_message_get_response(
            OutgoingMessage(OutgoingMessageType.stop_livestream, serial_no=serial_no)
        )

    async def _get_is_p2p_streaming(
        self, product_type: ProductType, serial_no: str
    ) -> bool:
        result = await self._send_message_get_response(
            OutgoingMessage(OutgoingMessageType.is_livestreaming, serial_no=serial_no)
        )
        return result[MessageField.LIVE_STREAMING.value]
    ...
    async def _get_voices(self, product_type: ProductType, serial_no: str) -> dict:
        result = await self._send_message_get_response(
            OutgoingMessage(
                OutgoingMessageType.get_voices,
                domain=product_type.name,
                serial_no=serial_no,
            )
        )
    ...
    async def quick_response(
        self, product_type: ProductType, serial_no: str, voice_id: int
    ) -> None:
    ...
    async def verify_pin(
        self, product_type: ProductType, serial_no: str, pin: str
    ) -> None:
        """verify pin for safe product"""
        await self._send_message_get_response(
            OutgoingMessage(
                OutgoingMessageType.verify_pin, serial_no=serial_no, pin=pin
            )
        )
    ...
    async def unlock(self, product_type: ProductType, serial_no: str) -> None:
        ...
        await self._send_message_get_response(
            OutgoingMessage(OutgoingMessageType.unlock, serial_no=serial_no)
        )
    ...
    async def chime(
        ...
        await self._send_message_get_response(
            OutgoingMessage(
                OutgoingMessageType.chime, serial_no=serial_no, ringtone=ringtone
            )
        )
    ...
    async def reboot(self, product_type: ProductType, serial_no: str) -> None:
        """Process reboot call"""
        await self._send_message_get_response(
            OutgoingMessage(OutgoingMessageType.reboot, serial_no=serial_no)
        )

    async def _on_message(self, message: dict) -> None:
        _LOGGER.debug(
            "Received bridge message type %s",
            message.get(MessageField.TYPE.value, "unknown"),
        )
        if message[MessageField.TYPE.value] == IncomingMessageType.result.name:
            future = self._result_futures.get(
                message.get(MessageField.MESSAGE_ID.value, -1), None
            )
            ...
            future.set_exception(
                FailedCommandException(
                    message[MessageField.MESSAGE_ID.value],
                    message[MessageField.ERROR_CODE.value],
                    message,
                )
            )
        elif message[MessageField.TYPE.value] == IncomingMessageType.event.name:
            event: Event = Event(
                type=message[IncomingMessageType.event.name][
                    IncomingMessageType.event.name
                ],
                data=message[IncomingMessageType.event.name],
            )
            ...
            if SCHEMA_VERSION > message["maxSchemaVersion"]:
                raise IncompatibleVersionException(
                    message["maxSchemaVersion"], SCHEMA_VERSION
                )
        else:
            raise UnexpectedMessageTypeException(message)

    async def _handle_event(self, event: Event):
        if event.data[MessageField.SOURCE.value] in [
            EventSourceType.station.name,
            EventSourceType.device.name,
        ]:
            ...
            try:
                product = self.__dict__[plural_product][
                    event.data[MessageField.SERIAL_NO.value]
                ]
                await product.process_event(event)
            except (KeyError, TypeError) as exc:
                raise DeviceNotInitializedYetException(event) from exc
        elif event.data[MessageField.SOURCE.value] in [
            EventSourceType.driver.name,
            EventSourceType.server.name,
        ]:
    ...
    async def _on_error(self, error: str) -> None:
        _LOGGER.error("Bridge WebSocket error: %s", type(error).__name__)
