...
async def async_setup_entry(
    ...
    product_properties = []
    for product in list(coordinator.devices.values()) + list(
        coordinator.stations.values()
    ):
        for command in ProductCommand:
            ...
            product_properties.append(
                Metadata.parse(
                    ...
                    {
                        "name": command.name,
                        "label": command.value.description,
    ...
    entities = [
        EufySecurityButtonEntity(coordinator, metadata)
